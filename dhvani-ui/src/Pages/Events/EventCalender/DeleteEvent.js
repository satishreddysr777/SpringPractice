...
const DeleteEvent = ({ open, setOpen, events = [], getAllEvents = () => {} }) => {
    ...
    const [deleteEventResults, setDeleteEventResults] = useState({
        loading: false,
        error: false,
        message: "",
        eventName: '',
    ...
    const deleteEvent = async () => {
        ...
        try {
            ...
            let res = await ApiService.deleteEvent(selectedEvent.value)
            setDeleteEventSuccess(true)
            setSelectedEvent(null)
            getAllEvents()
            setDeleteEventResults(prev => ({
                ...prev,
                "loading": false,
                    ...
                    <div className="flex min-h-full items-start justify-center p-4 sm:mt-[6%] sm:p-0">
                        ...
                        >
                            <Dialog.Panel className="relative transform overflow-unset rounded-md bg-white text-left shadow-md transition-all sm:my-8 w-full sm:max-w-[40em] h-full">
                                ...
                                {deleteEventResults.error && (
                                    <div
                                        className="bg-red-100 rounded-md py-2 px-6 mb-4 mx-[2rem] text-base text-center text-red-700"
                                        role="alert"
                                    >
                                        {deleteEventResults.message}
                                    </div>
                                )}

                                {!deleteEventResults.error && deleteEventSuccess && (
                                    <div
                                        className="bg-green-100 rounded-md py-2 px-6 mb-4 mx-[2rem] text-base text-center text-rgreened-700"
                                        role="alert"
                                    >
                                        {deleteEventResults.eventName} - {deleteEventResults.message}
                                    </div>
                                )}
