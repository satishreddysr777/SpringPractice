...
const UpcomingEvents = () => {
    ...
    const getUpcomingEvents = async () => {
        try {
            ...
            let res = await ApiService.getUpcomingEvents()
            setEvents(prev => ({
                ...prev.data.push,
                'loading': false,
        ...
        <section>
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:max-w-[100rem] lg:px-8 mt-10 xs:flex-col justify-between md:flex">
                ...
                <div className="flex justify-between md:block mt-5 md:mt-0">
                    <button
                        onClick={() => navigateTo('/current-events')}
            ...
            <div className="mx-auto max-w-7xl py-2 px-4 sm:px-6 lg:max-w-[100rem] lg:px-8">
                <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                    {events.data.map((event, index) => (
                        <EventCard event={event} key={index} refresh={getUpcomingEvents} />
                    ))}
                </div>
